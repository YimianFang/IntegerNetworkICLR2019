from ICLR_integermodel import IntegerModel, save_model, load_resume_model, load_pretrain_model
import os
import argparse
import torch
...
parser.add_argument('-r', '--resume', default='', help='load quantized model')
...
if __name__ == "__main__":
    ...
    model = IntegerModel(out_channel_N, out_channel_M)
    if args.pretrain != '':
        logger.info("loading model:{}".format(args.pretrain))
        global_step = load_pretrain_model(model, args.pretrain)
        global_step = 0
    elif args.resume != '':
        logger.info("loading model:{}".format(args.resume))
        global_step = load_resume_model(model, args.resume)
    net = model.cuda() ###cuda
    # net = model.cpu() ###cpu
